...
`nuthatch.stl` is the **full-size rev H aircraft**, generated by
`analysis/geometry-mesh.py` — cage, glazing, cowl and boom all modelled
explicitly. Good for visualising and checking clearances; not editable. The
editable frame is `analysis/frame.py`, exported to `cad/`.

The rest of this file is the quarter-scale validation aeroplane.
